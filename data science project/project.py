...
while plotline and titleline:
    ...
    if shecount>0 and shecount<5:
        smallshecount+=1
    if hecount>=5 and hecount<10:
        medhecount+=1
    if shecount>=5 and shecount<10:
        medshecount+=1
    if shecount>=10 and shecount<50:
        largeshecount+=1
    if hecount>=10: # and hecount<50:
        largehecount+=1
...
plt.title('Counts by gender (all titles)')
plt.xticks(index + bar_width, ('count=0', '0<count<5', '5<=count<10', '10<=count'))
plt.legend()
...
plt.xlabel('Range of counts')
plt.ylabel('Number of titles')
plt.title('Counts by gender (TV titles)')
plt.xticks(index + bar_width, ('count=0', '0<count<5', '5<=count<10', '10<=count'))
plt.legend()
#plt.tight_layout()
